...
def({ id:'sa', title:'Spectrum Analyzer', cat:'Analysis',
  ...
  process(n,I){
    ...
    //
    // Исключение — n._dragPending: пока тащат окно за реально захваченный край (см. drag в draw()),
    // fmin/fmax НАРОЧНО выставляются СРАЗУ, оптимистично, на страницу вперёд реальных данных —
    // не дожидаясь, пока приёмник реально перестроится (USB асинхронно). Раньше ждали подтверждения
    // (specSpan реально сдвинулся) и только потом двигали окно — но если мышь отпускали ДО того, как
    // подтверждение приходило, окно так и оставалось прижатым к СТАРОМУ краю, (fmin+fmax)/2 снова
    // указывал на старое место, и steerFreq тянул приёмник назад ("дрыгается туда-сюда и остаётся
    // там же было"). Теперь окно двигается сразу и никогда само не откатывается — а эта проверка,
    // которая иначе увидела бы "окно не пересекается с (ещё старыми) данными" и схлопнула бы зум
    // обратно на весь охват, временно отключена, пока подтверждение не пришло.
    if(sp){
