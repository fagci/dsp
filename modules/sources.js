...
async function rtlReadLoop(n){
  ...
  // этого удвоения частоты — не из-за нехватки CPU как таковой. Один читатель + вдвое более
  // редкие (крупные) чтения — тот же эффект, что даёт увеличение BLOCK у самого движка: меньше
  // диспетчеризаций/структурных клонирований в секунду на ту же полезную работу.
  const isIQ=n.p.demod==='IQ';
  const READS_PER_SEC=isIQ?20:10;
