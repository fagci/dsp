...
async function rtlReadLoop(n){
  ...
  // следующее), и на зазоре между завершением одного чтения и стартом следующего терялась
  // пропускная способность вне зависимости от того, сколько оптимизируй сам воркер.
  ...
  async function readerLoop(){
    while(n.reading && n.dev){
      // размер чтения — как у референса: sourceRate/20 (по умолчанию 20 чтений/с на КАЖДОГО
      // читателя, т.е. суммарно ~40 чтений/с на двоих), округлено вверх до кратного 512.
      const chunkSamples=Math.max(512, Math.min(131072, 512*Math.ceil(n.sourceRate/20/512)));
...
def({ id:'rtlsdr', title:'RTL-SDR', cat:'Источники',
  ...
  process(n,I){
    const freqDriven=typeof I.freq==='number';
    if(freqDriven) setMod(n,'freq',I.freq);
    ...
    // НО: если вход 'freq' подключён — центром явно рулят напрямую (например, Тюнер), и он
    // применяется каждый тик безусловно (строка выше). Спорить за центр скачками в этом случае
    // нельзя: 'freq' на следующем же тике перетрёт скачок обратно, а tuneFreq попробует опять —
    // получается бесконечное перетягивание. Пока 'freq' активен, tuneFreq только доводит NCO
    // в пределах уже выбранной полосы, не трогая центр.
