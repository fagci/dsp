...
def({ id:'rtlsdr', title:'RTL-SDR', cat:'Sources',
  ...
  process(n,I){
    ...
    // "наматывается" плавно). rtlApplyPending всё равно не шлёт на USB больше одной команды разом
    // (см. n.busy там) — пока предыдущая перестройка не отработает, здесь просто копится САМОЕ
    // СВЕЖЕЕ значение want, и как только шина освобождается, приёмник прыгает сразу туда, а не
    // отрабатывает каждый промежуточный тик — так что частые вызовы here ничего не стоят лишнего.
